...
The hashtable implementation used in \hrc{src/interpreter/memory/hashtable.c}                   % TODO: maybe its own paragraph! interpolation search vs binary search
...
\item On lines 10 and 11, the standard library function \emph{std.print} is used
    ...
    that is print out signifies a so-called escape-sequence. The \emph{n} stands
    for a newline, but other letters such as a \emph{t} for a tabulator are possible.
\item Lastly, line 13 contains another calculation in which the variable \emph{count}
    ...
    it is simoultaneously its return value, e.g. the value of count is returned
    ...
    This means the inner function is executed exacly ten times, because
...
\emph{Bibliography} section on page \pageref{bibliography}.
