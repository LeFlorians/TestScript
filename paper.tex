...
\begin{titlepage}\begin{center}
    ...
    \Large
    \vspace{1.5cm}
    Matura Paper by\\ \textbf{Florian M. Donnelly}
...
\abstract{
    This project is about creating a programming language and an accompanying
    ...
    the different entities that make up an interpreter. 
...
% TODO: say sth about README.md and expand it, makefile
...
functions that are included. The standard library is a set of function
included with the interpreter that extend its functionality over the
available operators.

The file \hrc{mapop.gperf} is also a source file. It is used in conjunction with the GNU gperf
...
uses binary search to locate elements variable-sized arrays in the hash table's buckets.
...
SYMBOL type tokens are mapped to operators such as plus or minus by \hrc{src/implementation/mappings/mapop.c}
...
this paper uses a simpler deterministic finite-state machine tokenizer.
...
a variable, but rather make it unusable for future references. To sum up,
the function \emph{f} is still capable of using \emph{std.exec}, because
at the time \emph{f} was defined, \emph{std.exec} was still accessible.
